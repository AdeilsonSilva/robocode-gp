...
/**
 * This class represents the main genetic algorithm. It assumes that Robocode is
 * installed at C:\robocode, and writes all files to the
 * "C:\robocode\robots\sampleex" directory. The robocode.jar library must be
 * included in the build path (C:\robocode\libs\robocode.jar by default)
 *
 * @author Ted Hunter
 *
 */
public class RunGP {
    ...
    final static int POP_SIZE = 5,
            ...
            TOURNY_SIZE = 6,
            BATTLE_HANDICAP = 20;
    static double PROB_CROSSOVER = 0.85,
    ...
    static MetaBot pool[] = new MetaBot[POP_SIZE],
    ...
    public static void main(String args[]) throws CloneNotSupportedException {
        ...
        while (genCount < MAX_GENS) {
            ...
            if (pool[best].fitness > bestSoFar.fitness) {
                bestSoFar = pool[best];
            }
    ...
    private static void breedPool() throws CloneNotSupportedException {
    ...
    public static int tournamentSelect() {
        ...
        int bestLocal = subPool[0];
        for (int i = 1; i < size; i++) {
            if (pool[subPool[i]].fitness > pool[bestLocal].fitness) {
                bestLocal = subPool[i];
            }
        }
        return bestLocal;
